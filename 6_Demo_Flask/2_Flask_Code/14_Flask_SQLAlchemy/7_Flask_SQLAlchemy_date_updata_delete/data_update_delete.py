...
if __name__ == '__main__':
    ...
    #####修改
    # 直接查询修改(类名.查询.查询方式(过滤条件).更新({以字典的形式修改需要改的信息}))
    User.query.filter_by(name='wang').update({'name':'linux','email':'python@163.com'})
    # 操作完的数据要记得提交给数据
    db.session.commit()
    ...
    '''
    # 查询出要删除的那条数据
    data11=User.query.get(1)
    # 将这条数据信息使用delete(查询出的数据对象)
    db.session.delete(data11)
    # 提交给数据库
    db.session.commit()
    '''
